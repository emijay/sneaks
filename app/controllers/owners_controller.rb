class OwnersController < ApplicationController
    ...
    def show
      ...
      if !@filters && !@years
          ...
          @releases = Shoe.select(:release_year).order(:release_year => :desc).distinct
          if request.query_parameters[:sort] == "r_date"
            @shoes = Shoe.where(user_id: params[:id], release_year: @years).order(created_at: :desc)

          elsif request.query_parameters[:sort] == "o_date"
            @shoes = Shoe.where(user_id: params[:id], release_year: @years).order(created_at: :asc)

          elsif request.query_parameters[:sort] == "h_price"
            @shoes = Shoe.where(user_id: params[:id], release_year: @years).order( 'cost_price::integer DESC')

          elsif request.query_parameters[:sort] == "l_price"
            @shoes = Shoe.where(user_id: params[:id], release_year: @years).order('cost_price::integer ASC')

          else
            @shoes = Shoe.where(user_id: params[:id], release_year: @years)
          end
